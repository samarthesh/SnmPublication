<?php
	...
	if(isset($_POST['add'])){
		...
		if(isset($_FILES['pdf']) && $_FILES['pdf']['name'] != ""){
            ...
            if(!is_dir("bootstrap/pdf/". $title."/")) {
                mkdir("bootstrap/pdf/". $title ."/");
            }
		    ...
		    if(move_uploaded_file($_FILES["pdf"]["tmp_name"][$i],"bootstrap/pdf/". $title ."/".$filename)){
                $insert = "INSERT into pdf(pdfname,titlename) values('$filename','$title')";
		...
		if(mysqli_num_rows($findResult)==0){
			...
			$insertCat = "INSERT INTO category(categoryid, category_name) VALUES (NULL, '$category')";
...
?>
	<form method="post" action="admin_add.php" enctype="multipart/form-data">
		<table class="table">
			...
			<tr>
			<th>PDF</th>
				<td><input type="file" name="pdf[]" multiple></td>
			</tr>
				<th>Publisher</th>
