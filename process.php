<?php
	...
	$id=$customer['id'];
	$query="INSERT INTO purchase(custid,name,Email,tid,PurDate,PhoneNumber,price) VALUES('$id','$name','$mail', '$tid','$date','$phno','$price')";
	...
	foreach($_SESSION['cart'] as $isbn => $qty)
{
		...
		$query = "INSERT INTO purbooks (book_title,purbookid,custid) VALUES 
		('$book_title', '$rare','$id')";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Insert value false!" . mysqli_error($conn);
...
?>
	<p class="lead text-success" id="p">Your order will be verified by our Team..</p>
   <script>
