<?php
$conn = mysqli_connect("localhost", "root","", "rd");
	...
	function select4LatestBook($conn){
		$row = array();
		$query = "SELECT book_isbn, book_image, book_title, book_author, book_price FROM books ORDER BY book_isbn DESC";
...
if (!function_exists("getBookName")){
	function getBookName($conn,$pbid){
		$row = array();
		$query = "SELECT book_title FROM purbooks WHERE purbookid = '$pbid' ORDER BY book_title ASC";
			$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Can't retrieve data " . mysqli_error($conn);
			exit;
...
if (!function_exists("getpbid")){
	function getpbid($conn,$id){
		$row = array();
		$query = "SELECT pbid FROM purchase WHERE custid = '$id' AND Status = '1'";
			$result = mysqli_query($conn, $query);
		if(!$result){
			echo "NO BOOK IS PURCHASE YET " . mysqli_error($conn);
			exit;
		}
		for($i = 0; $i < 10; $i++){
			array_push($row, mysqli_fetch_assoc($result));
...
if (!function_exists("getbooktitle")){
	function getbooktitle($isbn){
		$conn = db_connect();
		$query = "SELECT book_title FROM books WHERE book_isbn = '$isbn'";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "get book price failed! " . mysqli_error($conn);
			exit;
